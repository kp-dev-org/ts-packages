...
export function mergeOpenApiDocs(
    ...
    const {
        ...
        description = `Combined API documentation from ${docs.length} services`,
        version = "1.0.0",
        conflictResolution = 'merge',
        prefixOperationIds = false,
        addServiceTags = false,
        removeUserIdHeaders = false
    } = options;    // Start with the first document as the base
    const merged: OpenAPIDocument = JSON.parse(JSON.stringify(docs[0]));
    ...
    for (let i = 1; i < docs.length; i++) {
        ...
        if (doc.servers) {
            ...
            doc.servers.forEach(server => {
                const exists = merged.servers!.some(existingServer =>
        ...
        if (doc.tags) {
            ...
            doc.tags.forEach(tag => {
                const exists = merged.tags!.some(existingTag =>
        ...
        if (doc.security) {
            ...
            doc.security.forEach(securityReq => {
                const exists = merged.security!.some(existingSecurity =>
                    JSON.stringify(existingSecurity) === JSON.stringify(securityReq)
                );
                if (!exists) {
...
function mergeComponents(
    target: NonNullable<OpenAPIDocument['components']>,
    source: NonNullable<OpenAPIDocument['components']>,
    serviceName: string
): void {
    Object.entries(source).forEach(([componentType, componentItems]) => {
        if (componentItems && typeof componentItems === 'object') {
            if (!target[componentType as keyof typeof target]) {
                target[componentType as keyof typeof target] = {};
            }

            const targetComponent = target[componentType as keyof typeof target] as Record<string, any>;
 ...
 */
function extractServiceName(doc: OpenAPIDocument): string {
 ...
 */
function isValidOpenAPIDocument(obj: any): obj is OpenAPIDocument {
    return (
        obj &&
        typeof obj === 'object' &&
 ...
 * ```
 */
export async function fetchAndMergeOpenApiDocs(
    urls: string[],
    options: MergeOptions = {},
 ...
 */
export function createEmptyOpenAPIDocument(
 ...
 */
function removeUserIdHeaderParameters(doc: OpenAPIDocument): void {
                    ...
                    operation.parameters = operation.parameters.filter((param: any) => {
                        ...
                        if (param && typeof param === 'object') {
                            ...
                            if (param.$ref && typeof param.$ref === 'string') {
                                ...
                                if (doc.components?.parameters?.[paramName]) {
